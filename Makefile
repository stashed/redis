...
crds_to_patch := installer.stash.appscode.com_stashredises.yaml
...
gen-crd-protos-%:
	...
	@docker run --rm                                     \
		...
		go-to-protobuf                                   \
			...
			--packages=-k8s.io/api/core/v1,stash.appscode.dev/redis/apis/$(subst _,/,$*)
...
gen-values-schema:
	@yq r api/crds/installer.stash.appscode.com_stashredises.v1.yaml spec.versions[0].schema.openAPIV3Schema.properties.spec > /tmp/stash-redis-values.openapiv3_schema.yaml
	@yq d /tmp/stash-redis-values.openapiv3_schema.yaml description > charts/stash-redis/values.openapiv3_schema.yaml

.PHONY: gen-chart-doc
gen-chart-doc: gen-chart-doc-stash-redis
...
chart-contents-%:
	...
	@if [ ! -z "$(CHART_VERSION)" ]; then                                                  \
		...
		yq w -i ./docs/examples/backup/backupconfiguration.yaml spec.task.name --tag '!!str' redis-backup-$(CHART_VERSION); \
		yq w -i ./docs/examples/restore/restoresession.yaml spec.task.name --tag '!!str' redis-restore-$(CHART_VERSION); \
	fi
